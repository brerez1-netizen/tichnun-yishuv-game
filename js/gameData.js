...
window.gameData = (function () {
  ...
  const STREETS = [
    ...
    // שני הרחובות שארז הוסיף וסימן בכחול בסבב נוסף - כביש היקפי מזרחי וכביש היקפי מערבי.
    [pt(842.9,139.0), pt(822.3,132.3), pt(821.2,135.0), pt(819.7,137.5), pt(817.8,139.7), pt(815.7,141.6), pt(813.2,143.1), pt(810.6,144.2), pt(807.7,144.9), pt(804.8,145.2), pt(795.9,179.4), pt(801.1,180.4), pt(804.2,181.4), pt(807.0,182.8), pt(809.5,184.7), pt(811.6,187.0), pt(813.3,189.6), pt(814.4,192.5), pt(815.0,195.6), pt(815.0,198.9), pt(816.2,208.7), pt(817.9,218.3), pt(820.0,227.9), pt(822.5,237.4), pt(827.0,250.9), pt(832.4,263.9), pt(838.7,276.6), pt(845.8,288.7), pt(853.7,300.4), pt(862.4,311.4), pt(871.9,321.9), pt(882.2,331.7), pt(884.8,334.6), pt(886.7,338.0), pt(887.9,341.6), pt(888.3,345.5), pt(920.5,358.2), pt(938.9,343.3), pt(931.6,340.0), pt(924.6,336.3), pt(911.2,328.1), pt(898.8,318.8), pt(887.3,308.5), pt(877.0,297.3), pt(867.7,285.3), pt(859.5,272.6), pt(852.5,259.2), pt(846.6,245.3), pt(842.1,230.9), pt(838.8,216.1), pt(836.8,200.9), pt(836.2,185.6), pt(837.0,170.1), pt(839.2,154.5), pt(840.9,146.8), pt(842.9,139.0)],
    [pt(232.1,35.0), pt(177.9,24.5), pt(17.6,452.4), pt(5.6,513.4), pt(34.9,519.0), pt(40.7,490.0), pt(40.7,489.2), pt(40.8,488.3), pt(40.9,487.5), pt(41.0,486.6), pt(42.8,477.0), pt(46.6,456.9), pt(50.5,447.3), pt(62.5,418.5), pt(74.5,389.7), pt(86.5,360.9), pt(98.1,333.0), pt(110.2,303.9), pt(123.6,271.7), pt(126.4,264.8), pt(139.4,233.5), pt(153.9,197.9), pt(155.5,193.9), pt(160.7,178.5), pt(184.0,113.1), pt(207.8,96.0), pt(221.2,75.8), pt(225.5,69.4), pt(232.1,35.0)],
  ];
  ...
  const RULES = [
    {
      ...
      title: "השלמת התכנון",
      check(plots) {
        const unassigned = plots.filter((p) => !p.landUse);
        return {
          ok: unassigned.length === 0,
          violatingIds: unassigned.map((p) => p.id),
          message:
            unassigned.length === 0
              ? "לכל המגרשים יש ייעוד - התכנון הושלם."
              : `טרם סיימת לתכנן - ${unassigned.length} מגרשים בלי ייעוד (מסומנים באדום). אי אפשר להשאיר שטחים לא מתוכננים בתב"ע - לכל מגרש חייב להיות ייעוד.`,
        };
      },
    },
    {
