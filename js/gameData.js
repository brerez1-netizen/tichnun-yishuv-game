...
window.gameData = (function () {
  ...
  // ---------- הרשת עצמה ----------
  // סיבוב שני: ארז צבע בעצמו את התשריט לפי שכבות - כחול=כבישים, ירוק=שצ"פ קיים,
  // חום=מבני ציבור קיימים - והריץ BOUNDARY על כולם, לא רק על המגרשים. זה נותן רחובות
  // אמיתיים (לא משוערכים ידנית!) ושטחים ירוקים/מבני ציבור קיימים בפועל, קבועים במפה
  // ולא ניתנים לעריכה - בדיוק כמו שהם בתשריט המאושר. ויתרנו על אפשרות שינוי רוחב רחוב
  // (זה כבר הרוחב האמיתי מהתשריט, אין למה "להרחיב או להצר"). מגרשי הבנייה ("rp") הם
  // היחידים שהסטודנט עורך.
  ...
  // שצ"פ קיים - שכבה ירוקה בתשריט. קבוע, לא אינטראקטיבי, נספר במניין השצ"פ הכולל.
  const PARK_POLYGONS = [
    [pt(267.0,348.1), pt(268.4,341.0), pt(214.5,330.7), pt(200.4,309.8), pt(203.5,293.6), pt(191.1,296.4), pt(181.2,304.2), pt(176.7,311.5), pt(193.2,314.7), pt(207.2,335.5), pt(196.8,389.4), pt(203.7,390.9)],
    [pt(364.1,648.3), pt(357.0,646.9), pt(350.6,680.2), pt(344.6,684.3), pt(337.5,721.5), pt(354.6,724.8), pt(361.8,687.6), pt(357.7,681.7)],
    [pt(514.8,500.7), pt(511.3,494.5), pt(458.4,527.8), pt(459.3,529.3), pt(462.1,533.6)],
    [pt(267.6,557.9), pt(269.0,550.7), pt(302.1,557.1), pt(300.9,564.2)],
    [pt(269.0,550.7), pt(265.0,544.8), pt(227.8,537.7), pt(224.6,555.1), pt(261.6,561.8), pt(267.6,557.9)],
    [pt(833.0,280.8), pt(845.5,300.9), pt(780.3,351.6), pt(771.5,337.2), pt(701.6,380.5), pt(697.7,374.3), pt(766.8,331.0), pt(758.0,315.9)],
    [pt(225.5,69.5), pt(221.2,75.8), pt(451.6,120.3), pt(450.6,118.5), pt(445.6,112.0)],
    [pt(503.1,123.0), pt(491.4,128.3), pt(613.6,151.5), pt(611.6,148.4), pt(602.6,142.2)],
    [pt(655.6,152.4), pt(648.4,153.2), pt(642.4,157.2), pt(797.2,187.2), pt(800.3,194.9), pt(808.1,199.9), pt(814.0,233.0), pt(816.0,239.9), pt(822.5,237.4), pt(817.9,218.3), pt(814.4,192.5), pt(811.6,187.0), pt(807.0,182.8), pt(801.1,180.4)],
    [pt(825.2,245.8), pt(818.7,248.4), pt(833.0,280.8), pt(855.3,313.7), pt(882.4,341.3), pt(888.3,346.0), pt(884.8,334.7), pt(863.9,313.1), pt(848.2,292.4), pt(835.2,269.9)],
    [pt(822.5,237.4), pt(816.0,239.9), pt(818.7,248.4), pt(825.2,245.8)],
  ];

  // מבני ציבור קיימים - שכבה חומה בתשריט. קבועים, לא אינטראקטיביים.
  const BUILDING_POLYGONS = [
    [pt(160.7,178.5), pt(232.2,192.2), pt(451.2,234.3), pt(453.6,129.1), pt(453.6,128.3), pt(453.6,127.4), pt(453.4,125.5), pt(453.0,123.7), pt(452.4,122.0), pt(451.6,120.3), pt(451.6,120.3), pt(451.1,119.3), pt(450.6,118.5), pt(450.0,117.6), pt(445.6,112.0), pt(225.5,69.5), pt(184.0,113.1)],
    [pt(389.4,443.6), pt(318.8,465.4), pt(309.1,471.4), pt(333.5,512.7), pt(418.1,638.6), pt(418.1,638.6), pt(418.1,638.6), pt(444.1,675.3), pt(445.7,674.1), pt(447.4,672.9), pt(449.0,671.7), pt(450.6,670.4), pt(474.8,651.2), pt(505.8,626.7), pt(488.6,605.5), pt(472.2,583.9), pt(456.5,561.7), pt(441.5,539.0), pt(427.3,515.8), pt(413.9,492.2), pt(401.2,468.1), pt(389.4,443.6)],
  ];
  ...
  const FIXED_TOTAL_AREA =
    PARK_POLYGONS.reduce((s, p) => s + polygonArea(p), 0) +
    BUILDING_POLYGONS.reduce((s, p) => s + polygonArea(p), 0);
  const FIXED_OPEN_AREA = PARK_POLYGONS.reduce((s, p) => s + polygonArea(p), 0);
  ...
  const RULES = [
    ...
    },
    {
      ...
      title: 'מינימום שצ"פ',
      check(plots) {
        const totalArea = plots.reduce((s, p) => s + p.area, 0) + FIXED_TOTAL_AREA;
        const openArea = plots.filter((p) => p.landUse === OPEN_SPACE_KEY).reduce((s, p) => s + p.area, 0) + FIXED_OPEN_AREA;
